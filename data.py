...
def randomShiftScaleRotate(
    ...
    if np.random.random() < u:
        ...
        mask = cv2.warpPerspective(mask, mat, (width, height), flags=cv2.INTER_LINEAR, borderValue=(0, 0, 0, ))
...
def default_loader(id, root) -> (np.ndarray, np.ndarray):
    ...
    img, mask = randomShiftScaleRotate(image=img,
                                       mask=mask,
                                       shift_limit=(-0.1, 0.1),
                                       scale_limit=(-0.1, 0.1),
    ...
    img, mask = randomHorizontalFlip(image=img, mask=mask)
    img, mask = randomVerticalFlip(image=img, mask=mask)
    img, mask = randomRotate90(image=img, mask=mask)

    mask = mask[..., np.newaxis]  # mask = np.expand_dims(mask, axis=2)
